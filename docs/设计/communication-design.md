...
通讯工具与其他业务工具地位平等，在 `prepareEntry.tools` 中声明。agent 在当前节点内可以主动调用它们。
...
| 配置位置 | 节点 `prepareEntry.tools` | Graph 共享环境 / 节点 `constraints` |
...
- 通讯层在节点完成时**选择性折叠**：未消费的订阅消息可保留到下一节点（取决于 Edge 的 `migrate.keep` 配置），已消费的消息清除
...
通讯不是 agent 的自由行为。agent 能否通讯、以哪些方式通讯，完全由**当前节点**通过 `prepareEntry` 和 `constraints` 决定。
