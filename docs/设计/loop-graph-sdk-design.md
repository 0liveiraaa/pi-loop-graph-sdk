...
SDK 的定位是**高度可定制化的 agent 编排框架**，而非声明式工作流引擎。所有扩展点都是函数（`execute`、`guard`、`migrate`、`validateCompletion`、`Entry.guard`、`custom router`），开发者可以自由组合代码逻辑与 LLM 推理，不预设任何 JSON schema 或声明式 DSL。

> 如果想获取开箱即用的声明式图定义体验，可在上层业务 SDK（如 pi-review-agent）中基于本框架封装声明层。框架本身专注于提供可靠、灵活、可观测的编排核心。
...
2. **高度可定制化**：所有扩展点都是函数，开发者自由组合代码与 LLM 推理，不预设声明式 DSL
...
### 修订 6：高度可定制化，不预设声明式 DSL

确定 SDK 的核心定位为函数式编排框架。开发者通过 execute/guard/migrate 等函数自由组合代码与 LLM 推理。声明式 DSL 由上层业务 SDK 自行封装。
