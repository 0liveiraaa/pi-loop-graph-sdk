...
框架通过语言层固化核心设计理念并提供默认便利（如上下文摘要），但在不违背核心机制的前提下，赋予开发者完全定制上下文流转（保留、抛弃或自定义）的绝对自由。
这段思想可以拆解为三个层次：
固化范式：通过语言规范贯彻设计理念，提供开箱即用的默认抽象（如默认的 Node summary 机制）。
保留逃生舱：不阻拦合理的定制化需求，允许开发者突破默认约定（如完全抛弃上下文）。
边界清晰：自由是有前提的——即“在不矛盾的情况下”。开发者可以改变数据的流转与记忆方式，但不能破坏图、节点、边的基础运行契约。
